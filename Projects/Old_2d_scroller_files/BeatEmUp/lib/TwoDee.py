...
class Sprite:
	...
	def flip_horizontal(self,frame_to_flip="-1"):
		...
		#	if frame_to_flip > self.lastSpriteFrame:
		#		frame_to_flip = self.lastSpriteFrame
		#	self.sprite_frame[frame_to_flip][0] = transform.flip(self.sprite_frame[frame_to_flip][0],True,False)
		#else:
			for i in range(self.totalSpriteFrames):
				self.sprite_frame[i][0] = transform.flip(self.sprite_frame[i][0],True,False)
			
	def flip_vertical(self,frame_to_flip="-1"):
		if frame_to_flip >= 0:
			if frame_to_flip > self.lastSpriteFrame:
				frame_to_flip = self.lastSpriteFrame
			self.sprite_frame[frame_to_flip][0] = transform.flip(self.sprite_frame[frame_to_flip][0],False,True)
		else:
			for i in range(self.totalSpriteFrames):
				self.sprite_frame[i][0] = transform.flip(self.sprite_frame[i][0],False,True)
	...
	def set_sprite_frame_range(self,start_range=0,end_range=-1):
		...
		self.totalSpritestoUse = (self.lastSpritetoUse - self.firstSpritetoUse) + 1
			
	
	def render(self):
			
			#check if a range has been set
			if (self.lastSpritetoUse != self.lastSpriteFrame) or (self.firstSpritetoUse != 0):		
				...
				self.displaySpriteFrame = int(float(self.currentSpriteFrame + self.spriteOffset) % (self.totalSpritestoUse))
				self.currentSpriteFrame = (int(math.ceil(  self.gameEnv.currentGameTimer.current_frame /  float(self.gameEnv.max_frame / self.totalSpritestoUse)  )) -1) 
				self.displaySpriteFrame = self.currentSpriteFrame + self.firstSpritetoUse
				if self.displaySpriteFrame > self.lastSpritetoUse:
					self.displaySpriteFrame = self.firstSpritetoUse
				#self.displaySpriteFrame = 1
			else:
